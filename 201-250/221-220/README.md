...
### 思考
对字符串的操作还是不灵活啊， 以后想要处理空格，就像是处理自己输入的数据的时候可以采用iostringstream类型啊。可以简化很多操作的。
...
public:
    vector<string> summaryRanges(vector<int>& nums) {
        ...
        for(int i = 0; i < nums.size();){
            ...
            if(end != start) result.push_back(to_string(nums[start]) + "->" + to_string(nums[end]));
